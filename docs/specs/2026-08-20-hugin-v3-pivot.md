...
Four findings from final review, against the built implementation rather than this spec:

1. **Deadline-sort dropped from the Applications view.** The sort options list in decision 2 (starred / deadline / updated / name) assumed each entry has a single deadline, but an Applications entry tracks a *company*, not an ad — a company can carry multiple ads with different deadlines, or none. There's no single deadline field to sort on without an entry-to-ad join. Deferred: an earliest-ad-deadline join (mirroring the Trenger-handling parent-chain match) would let "deadline" sort by the soonest deadline among a company's active ads, but that's new plumbing, not a rename. Applications currently sorts starred / updated / name only.
2. **CLI cannot star.** Decision 2's ⭐ Starred flag is API-backed (`PUT /api/pipeline/{orgnr}` accepts `starred: bool?`) and exposed as a dashboard gesture, but `hugin track` (decision 9) has no `--star` flag — starring stayed a dashboard-only affordance. Deferred, not forgotten: adding the flag is a small CLI change once it's clear starring is wanted outside the browser.
3. **Server warnings render bokmål in the EN UI.** Decision 9 keeps CLI text bokmål; the same warning strings (e.g. the missing-begrunnelse message) are also what `PipelineService`/the track API return, and the web UI surfaces them verbatim regardless of the active language toggle. Accepted: these are operator-facing edge messages, not primary UI copy, and duplicating them into the i18n tables would split one source of truth into two that must stay in sync for little benefit.
4. **Known deferred minors**, none blocking: the pre-migration `.bak` copy (Loopholes, decision-notes) captures the SQLite file but not an uncheckpointed `-wal` file, so a crash between the `.bak` copy and the migration's checkpoint could leave the backup missing very recent writes; `EksportView`'s scope/format preview has no stale-response guard against a fast double-change of inputs; the dashboard star button has no in-flight disable, so a double-click can fire two overlapping requests; and the regular (non-single-exe) publish output still carries the embedded `wwwroot` manifest copy alongside the physical one (~537KB dead weight — the physical copy wins at startup per decision 8, the embedded copy simply never gets picked).
